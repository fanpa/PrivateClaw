...
export function createBrowserAuthTool() {
  return {
    ...
    tool: {
      description: 'Open Chrome/Edge browser to a login page. The user logs in manually, then cookies are captured and returned. Use set_header to save specific cookie values or tokens to config. Pass extraHeaders to inject custom HTTP headers into every browser request (e.g. {"User-Agent": "CustomBot/1.0"}).',
    ...
    execute: async (params: { url: string; extraHeaders?: Record<string, string> }): Promise<BrowserAuthResult> => {
      try {
        const result = await executeAuth({ url: params.url, extraHeaders: params.extraHeaders });
        return {
          domain: result.domain,
          cookies: result.cookies.map((c) => ({ name: c.name, value: c.value })),
          message: `Captured ${result.cookies.length} cookies for ${result.domain}. Review the cookies and use set_header to save the needed values.`,
