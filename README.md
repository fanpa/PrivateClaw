...
`shell_exec`를 통한 도메인 제한 우회(예: `curl`, `wget`)를 방지하기 위해, 실행 가능한 명령어를 화이트리스트로 제한할 수 있습니다. 사용 중인 OS에 맞는 명령어를 설정하세요.
...
JSON 파일 기반 세션 저장 및 복원 기능을 제공합니다. 각 세션은 개별 JSON 파일로 저장되어 이전 대화를 이어서 진행할 수 있습니다.
